package zhutong

import (
	"context"
	"crypto/md5"
	"encoding/hex"
	"encoding/json"
	"errors"
	"net/http"
	"strconv"
	"strings"
	"time"

	"github.com/CrazyLionCat/sms4go/provider"
	"github.com/CrazyLionCat/sms4go/provider/base"
	"github.com/CrazyLionCat/sms4go/sms"
)

const Supplier = "zhutong"

type Client struct {
	config sms.BaseConfig
	http   *base.HTTPClient
}

func init() {
	provider.Register(Supplier, func(config sms.BaseConfig) (sms.Client, error) {
		return New(config)
	})
}

func New(config sms.BaseConfig) (*Client, error) {
	config = withDefaults(config)
	httpClient, err := base.NewHTTPClient(config)
	if err != nil {
		return nil, err
	}
	return &Client{config: config, http: httpClient}, nil
}

func (c *Client) ConfigID() string { return c.config.ID() }

func (c *Client) Supplier() string { return Supplier }

func (c *Client) SendMessage(ctx context.Context, phone string, message string) (*sms.Response, error) {
	if c.config.Signature == "" || c.config.TemplateID == "" || c.config.TemplateName == "" {
		return c.sendText(ctx, []string{phone}, message)
	}
	return c.SendTemplate(ctx, phone, c.config.TemplateID, map[string]string{c.config.TemplateName: message})
}

func (c *Client) SendMessageWithParams(ctx context.Context, phone string, messages map[string]string) (*sms.Response, error) {
	return c.SendTemplate(ctx, phone, c.config.TemplateID, messages)
}

func (c *Client) SendTemplate(ctx context.Context, phone string, templateID string, messages map[string]string) (*sms.Response, error) {
	return c.sendTemplate(ctx, []string{phone}, templateID, messages)
}

func (c *Client) MassTexting(ctx context.Context, phones []string, message string) (*sms.Response, error) {
	if c.config.Signature == "" || c.config.TemplateID == "" || c.config.TemplateName == "" {
		return c.sendText(ctx, phones, message)
	}
	return c.MassTextingTemplate(ctx, phones, c.config.TemplateID, map[string]string{c.config.TemplateName: message})
}

func (c *Client) MassTextingTemplate(ctx context.Context, phones []string, templateID string, messages map[string]string) (*sms.Response, error) {
	return c.sendTemplate(ctx, phones, templateID, messages)
}

func (c *Client) sendText(ctx context.Context, phones []string, content string) (*sms.Response, error) {
	if len(phones) == 0 || len(phones) >= 2000 {
		return nil, sms.ErrInvalidPhone
	}
	if content == "" || len(content) >= 1000 {
		return nil, sms.ErrEmptyMessage
	}
	if !strings.Contains(content, "【") {
		return nil, errors.New("sms4go: zhutong text message content must contain signature")
	}
	tKey := time.Now().Unix()
	payload := map[string]any{
		"username": c.config.AccessKeyID,
		"password": md5Hex(md5Hex(c.config.AccessKeySecret) + strconvFormatInt(tKey)),
		"tKey":     strconvFormatInt(tKey),
		"mobile":   strings.Join(phones, ","),
		"content":  content,
	}
	return c.doPost(ctx, c.config.RequestURL+"v2/sendSms", payload)
}

func (c *Client) sendTemplate(ctx context.Context, phones []string, templateID string, messages map[string]string) (*sms.Response, error) {
	signature := c.config.Signature
	if signature == "" {
		return nil, errors.New("sms4go: zhutong template signature is empty")
	}
	if !strings.HasPrefix(signature, "【") {
		signature = "【" + signature
	}
	if !strings.HasSuffix(signature, "】") {
		signature += "】"
	}
	tKey := time.Now().Unix()
	records := make([]map[string]any, 0, len(phones))
	for _, phone := range phones {
		records = append(records, map[string]any{"mobile": phone, "tpContent": messages})
	}
	payload := map[string]any{
		"username":  c.config.AccessKeyID,
		"password":  md5Hex(md5Hex(c.config.AccessKeySecret) + strconvFormatInt(tKey)),
		"tKey":      tKey,
		"tpId":      templateID,
		"signature": signature,
		"ext":       "",
		"extend":    "",
		"records":   records,
	}
	return c.doPost(ctx, c.config.RequestURL+"v2/sendSmsTp", payload)
}

func (c *Client) doPost(ctx context.Context, endpoint string, payload any) (*sms.Response, error) {
	var last *sms.Response
	var lastErr error
	for attempt := 0; attempt <= c.config.MaxRetries; attempt++ {
		if attempt > 0 {
			time.Sleep(c.retryInterval())
		}
		raw, _, err := c.http.PostJSON(ctx, endpoint, payload, http.Header{})
		if err != nil {
			lastErr = err
			continue
		}
		var data map[string]any
		if err := json.Unmarshal(raw, &data); err != nil {
			return nil, err
		}
		last = sms.Failure(data, c.ConfigID())
		if code, ok := numericCode(data["code"]); ok && code <= 200 {
			last.Success = true
			return last, nil
		}
	}
	return last, lastErr
}

func numericCode(value any) (int, bool) {
	switch typed := value.(type) {
	case float64:
		return int(typed), true
	case int:
		return typed, true
	default:
		return 0, false
	}
}

func md5Hex(text string) string {
	sum := md5.Sum([]byte(text))
	return hex.EncodeToString(sum[:])
}

func strconvFormatInt(value int64) string {
	return strconv.FormatInt(value, 10)
}

func (c *Client) retryInterval() time.Duration {
	if c.config.RetryInterval <= 0 {
		return 5 * time.Second
	}
	return c.config.RetryInterval
}

func withDefaults(config sms.BaseConfig) sms.BaseConfig {
	config.Supplier = Supplier
	if config.RequestURL == "" {
		config.RequestURL = "https://api.mix2.zthysms.com/"
	}
	return config
}
